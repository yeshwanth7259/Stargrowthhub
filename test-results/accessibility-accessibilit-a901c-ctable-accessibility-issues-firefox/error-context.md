...
```ts
  ...
  3  | 
  4  | test('Homepage should not have any automatically detectable accessibility issues', async ({ page }) => {
  5  |   await page.goto('/');
  6  |   // Wait for animations to settle
...
> 9  |   const accessibilityScanResults = await new AxeBuilder({ page }).analyze();
  ...
  12 |     console.log(accessibilityScanResults.violations);
  13 |   }
  14 |   
  15 |   // Soft assertion so it doesn't hard-fail immediately if there are minor contrast issues
